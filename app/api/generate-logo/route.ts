...
async function uploadFileToS3(
  ...
  const objectKey = `reference-logos/${userId}/${timestamp}-${index}-${file.name}`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!logoName || !description) {
      return NextResponse.json(
        {
          success: false,
          message: "Logo name and description are required",
        },
        { status: 400 }
      );
    }

    // Handle reference logos (optional)
    let logoUrls: string[] = [];
    ...
    // Trigger Inngest function for logo generation with URLs
    ...
    return NextResponse.json({
      ...
      data: {
        eventId,
        status: "PROCESSING",
        message: "Your logos are being generated. This may take a few minutes.",
      },
    });
  } catch (error) {
